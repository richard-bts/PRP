...
namespace PRP.WinService.ApiServices
{
    public class PRPService : BaseService, IPRPService
    {
        ...
        public async Task<ResponseDto?> GetPODReport(DateTime inputDate, int clientID)
        {
            ...
            {
                Data = inputDate,
                Url =  $"{_Configuration.GetSection("ServiceUrl:PRP.Service.Api").Value}api/GetPODReport?inputDate={inputDate}&clientID={clientID}"
            });           
        }
        public async Task<ResponseDto?> GetScanReport(DateTime inputDate, int clientID)
        {
            ...
            {
                Data = inputDate,
                Url = $"{_Configuration.GetSection("ServiceUrl:PRP.Service.Api").Value}api/GetScanReport?inputDate={inputDate}&clientID={clientID}"
            });
        }
        public async Task<ResponseDto?> GetExceptionReport(DateTime inputDate, int clientID)
        {
            ...
            {
                Data = inputDate,
                Url = $"{_Configuration.GetSection("ServiceUrl:PRP.Service.Api").Value}api/GetExceptionReport?inputDate={inputDate}&clientID={clientID}"
            });
        }
        public async Task<ResponseDto?> GetPartners()
        {
            return await this.SendAsync(new ApiRequest()
            {
                Url = $"{_Configuration.GetSection("ServiceUrl:PRP.Service.Api").Value}api/GetPartners"
            });
        ...
        public async Task<ResponseDto?> GetPartnerEmails(int partnerID)
        {
            ...
            {
                Data = partnerID,
                Url = $"{_Configuration.GetSection("ServiceUrl:PRP.Service.Api").Value}api/GetPartnerEmails?partnerId={partnerID}"
            });
